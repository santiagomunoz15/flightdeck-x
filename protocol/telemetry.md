...
Each UDP datagram contains exactly one 163-byte telemetry packet. Multi-byte
...
| 4 | 2 | `version` | `uint16` | — | Protocol version; currently `2` |
...
| 91 | 8 | `gimbal_command_deg` | 2 × `float32` | deg | East and north engine-gimbal commands |
| 99 | 8 | `grid_fin_command_deg` | 2 × `float32` | deg | East and north aerodynamic-control commands |
| 107 | 4 | `fault_flags` | `uint32` | — | Active faults; zero means nominal |
| 111 | 24 | `truth_position_m` | 3 × `float64` | m | Noise-free world-frame simulation position |
| 135 | 24 | `truth_velocity_mps` | 3 × `float64` | m/s | Noise-free world-frame simulation velocity |
| 159 | 4 | `crc32` | `uint32` | — | IEEE CRC-32 of bytes 0–158 |
...
`0xFFFFFFFF`, and final XOR `0xFFFFFFFF`. It covers the first 159 bytes of the
...
they are never decoded using the version 2 layout.
